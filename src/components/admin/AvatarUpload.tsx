...
interface CropState {
  ...
  resizing: boolean;
  resizeStart: { mx: number; my: number; size: number } | null;
  naturalW: number;
  naturalH: number;
  displayW: number;
  displayH: number;
...
export default function AvatarUpload({ currentUrl, onUploaded }: Props) {
  ...
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    img.onload = () => {
      ...
      setCrop({ src, x, y, size, dragging: false, dragStart: null, resizing: false, resizeStart: null, naturalW: img.naturalWidth, naturalH: img.naturalHeight, displayW, displayH });
    };
    img.src = src;
  ...
  const onMouseDown = useCallback((e: React.MouseEvent, mode: "drag" | "resize") => {
    ...
    setCrop((prev) => {
      ...
      return { ...prev, resizing: true, resizeStart: { mx: e.clientX, my: e.clientY, size: prev.size } };
    });
  }, []);
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    setCrop((prev) => {
      ...
      if (prev.dragging && prev.dragStart) {
        const dx = e.clientX - prev.dragStart.mx;
        const dy = e.clientY - prev.dragStart.my;
        return {
          ...prev,
          x: clamp(prev.dragStart.x + dx, 0, prev.displayW - prev.size),
          y: clamp(prev.dragStart.y + dy, 0, prev.displayH - prev.size),
        };
      }
      if (prev.resizing && prev.resizeStart) {
        const delta = e.clientX - prev.resizeStart.mx;
        const newSize = clamp(prev.resizeStart.size + delta, 60, Math.min(prev.displayW - prev.x, prev.displayH - prev.y));
  ...
  const confirm = async () => {
    ...
    try {
      // Draw cropped region to a 400x400 canvas
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.85)" }}>
          <div className="rounded-2xl p-6 max-w-lg w-full mx-4" style={{ background: "#1A1810", border: "1px solid #2E2A18" }}>
            ...
            >
              ...
              {/* Dark overlay — bottom */}
              <div className="absolute inset-x-0" style={{ top: crop.y + crop.size, bottom: 0, background: "rgba(0,0,0,0.55)" }} />
              {/* Dark overlay — left */}
              <div className="absolute" style={{ top: crop.y, left: 0, width: crop.x, height: crop.size, background: "rgba(0,0,0,0.55)" }} />
              {/* Dark overlay — right */}
              <div className="absolute" style={{ top: crop.y, left: crop.x + crop.size, right: 0, height: crop.size, background: "rgba(0,0,0,0.55)" }} />
